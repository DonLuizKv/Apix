...
interface SaveRequestModalProps {
  ...
  onSaveMultiple: (collectionName: string, requests: Array<{ name: string, method: HttpMethod }>) => void
  currentUrl: string
...
export function SaveRequestModal({
  ...
  const updateRequest = (id: string, field: 'name' | 'method', value: string) => {
    setMultipleRequests(multipleRequests.map(req =>
      ...
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a2a] bg-gradient-to-r from-[#1a1a1a] to-[#1f1f1f]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-foreground">Guardar Petición</h2>
              <p className="text-xs text-muted-foreground">Organiza tus requests en colecciones</p>
            </div>
          </div>
          <Button
            variant="ghost"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div className="flex gap-2 p-1 bg-[#0d0d0d] rounded-lg border border-[#2a2a2a]">
            <button
              ...
              className={`flex-1 px-4 py-2.5 rounded-md text-sm font-medium transition-all ${mode === 'single'
            ...
            <button
              ...
              className={`flex-1 px-4 py-2.5 rounded-md text-sm font-medium transition-all ${mode === 'multiple'
                  ? 'bg-primary text-primary-foreground shadow-sm'
                  : 'text-muted-foreground hover:text-foreground hover:bg-secondary/30'
                }`}
            >
            ...
            <div className="space-y-2 p-4 bg-[#0d0d0d] rounded-lg border border-[#2a2a2a]">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded ${currentMethod === 'GET' ? 'bg-emerald-500/20 text-emerald-400' :
                      currentMethod === 'POST' ? 'bg-amber-500/20 text-amber-400' :
                        currentMethod === 'PUT' ? 'bg-blue-500/20 text-blue-400' :
                          currentMethod === 'DELETE' ? 'bg-red-500/20 text-red-400' :
            ...
            {collections.length > 0 && (
              <div className="flex gap-2 p-1 bg-[#0d0d0d] rounded-lg border border-[#2a2a2a]">
                <button
                  ...
                  className={`flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all ${!useExisting
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
                {multipleRequests.map((req, index) => (
                  ...
                  >
                    ...
                    <Select
                      value={req.method}
                      onValueChange={(value: string) => updateRequest(req.id, 'method', value)}
                    >
                      ...
                      <SelectContent className="bg-[#1a1a1a] border-[#2a2a2a]">
                        {HTTP_METHODS.map((method) => (
                          <SelectItem key={method} value={method} className="text-xs font-semibold">
                            <span className={`${method === 'GET' ? 'text-emerald-400' :
        ...
        <div className="flex justify-between items-center gap-3 px-6 py-4 border-t border-[#2a2a2a] bg-gradient-to-r from-[#1a1a1a] to-[#1f1f1f]">
          <p className="text-xs text-muted-foreground">
            {mode === 'multiple' ? `${multipleRequests.length} petición(es)` : 'Petición individual'}
          </p>
